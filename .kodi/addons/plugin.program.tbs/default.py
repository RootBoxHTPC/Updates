...
codename         =  ADDON.getSetting('codename')
...
ARTPATH          =  'http://totalrevolution.tv/art/' + os.sep
...
oeservicpath     =  xbmc.translatePath('special://home/addons/service.openelec.settings')
...
#    baseascii='687474703a2f2f746f74616c78626d632e636f6d2f746f74616c7265766f6c7574696f6e2f4164646f6e506f7274616c2f6164646f6e64657461696c735f746573742e7068703f69643d2573'
#    BaseURL=binascii.unhexlify(baseascii) % (url)
...
#        asciiinc = '687474703a2f2f746f74616c78626d632e636f6d2f746f74616c7265766f6c7574696f6e2f4164646f6e506f7274616c2f646f776e6c6f6164636f756e742e7068703f69643d2573'
#        incremental=binascii.unhexlify(asciiinc) % (addon_id)
        ...
        if modulestatus != 0 and repostatus != 0 and forum != 'None':
            dialog.ok(name+" Install Complete",'Please support the developer(s) [COLOR=dodgerblue]'+provider_name,'[/COLOR]Support for this add-on can be found at [COLOR=yellow]'+forum,'[/COLOR][CR]Visit [COLOR=lime][B]www.totalxbmc.tv[/COLOR][/B] for all your Kodi needs.')
...
def Addon_Install_NA(name,contenttypes,repo_link,repo_id,addon_id,provider_name,forum,data_path):
    ...
    if os.path.exists(repopath):
        ...
        choice = dialog.yesno('WARNING!','[COLOR=gold]This Add-on may be unlawful in your region.[/COLOR][CR]The repository required for installation of this add-on has been detected on your system. Would you like to continue to the Kodi addon browser to install?')
        ...
        dialog.ok('WARNING!','[COLOR=gold]This add-on may possibly be unlawful in your region.[/COLOR][CR]If you\'ve investigated the legality of it and are happy to install then you must have the following repository installed: [COLOR=dodgerblue]'+repo_id+'[/COLOR]')
...
def Addon_Menu(sign):
    if trcheck == 'true':
        o0oO('',codename+'[COLOR=darkcyan] Keyword Install[/COLOR]', keywordpath, 'keywords', 'Keywords.png','','','')
    o0oO('folder','[COLOR=grey][Manual Search][/COLOR] Search By Name','pass='+sign+'&name=','search_addons','Search_Addons.png','','','')
    o0oO('folder','[COLOR=grey][Manual Search][/COLOR] Search By Author','pass='+sign+'&author=','search_addons','Search_Addons.png','','','')
    o0oO('folder','[COLOR=grey][Manual Search][/COLOR] Search In Description','pass='+sign+'&desc=','search_addons','Search_Addons.png','','','')
    o0oO('folder','[COLOR=grey][Manual Search][/COLOR] Search By Add-on ID','pass='+sign+'&addonid=','search_addons','Search_Addons.png','','','')
    ...
    o0oO('','[COLOR=gold][Kodi Add-on Browser][/COLOR] Install From Zip','','install_from_zip','Search_Addons.png','','','')
    o0oO('','[COLOR=gold][Kodi Add-on Browser][/COLOR] Browse My Repositories','','browse_repos','Search_Addons.png','','','')
    o0oO('','[COLOR=gold][Kodi Add-on Browser][/COLOR] Check For Add-on Updates','','check_updates','Search_Addons.png','','','')
...
#Create backup menu
def Backup_Option():
    o0oO('','[COLOR=dodgerblue]Create My Own Full Backup[/COLOR]','local','local_backup','Backup.png','','','Back Up Your Full System')
...
def Backup_Restore():
    if os.path.exists(oeservicpath):
        o0oO('','Create OpenELEC Backup','none','openelec_backup','Backup.png','','','')
    o0oO('folder','Backup My Content (FULL BACKUP)','none','backup_option','Backup.png','','','')
...
def Categories(localbuildcheck,localversioncheck,id,welcometext):
            ...
            if id != 'Local':
                ...
                if updatecheck == True:
                    ...
                    if not 'Partially installed' in localbuildcheck:
                        o0oO('folder','[COLOR=dodgerblue]'+localbuildcheck+':[/COLOR] [COLOR=lime]NEW VERSION AVAILABLE[/COLOR]',id,'showinfo','TOTALXBMC.png','','','')
                    
                    if '(Partially installed)' in localbuildcheck:
                        o0oO('folder','[COLOR=lime]Current Build Installed: [/COLOR][COLOR=dodgerblue]'+localbuildcheck+'[/COLOR]',id,'showinfo2','TOTALXBMC.png','','','')
                else:
                    o0oO('folder','[COLOR=lime]Current Build Installed: [/COLOR][COLOR=dodgerblue]'+localbuildcheck+'[/COLOR]',id,'showinfo','TOTALXBMC.png','','','')
            
            else:
                
                if localbuildcheck == 'Incomplete':
                    o0oO('','[COLOR=lime]Your last restore is not yet completed[/COLOR]','url',Check_Local_Install(),'TOTALXBMC.png','','','')
    ...
    if addonportal == 'true':
        o0oO('folder','Add-on Portal',sign,'addonmenu', 'Search_Addons.png','','','')
...
# over whole file otherwise skin will update and break
    ...
    portalcontent   = Open_URL('http://totalxbmc.com/totalrevolution/AddonPortal/approved.php')
        ...
        if not 'totalinstaller' in name and not 'plugin.program.tbs' in name and not 'packages' in name and not 'repo.' in name and not 'repository' in name:
            ...
            if name in portalcontent:
                ...
                if reponame!="repository.xbmc.org" and not os.path.exists(os.path.join(ADDONS,reponame)):
                    print"### Cannot install "+name+" - you need the following repo pre-installed: "+reponame
                    writefile.write('[COLOR=red][FAILED][/COLOR] '+name+" - you need the following repo pre-installed: [COLOR=gold]"+reponame+'[CR][/COLOR]')
            
            else:
                
                if not os.path.exists(os.path.join(ADDONS,name)):
                    choice=dialog.yesno('[COLOR=gold]Unknown Add-on Detected[/COLOR]','This build contains content that can\'t be verified','against the Addon Portal. Would you like to install:[COLOR=dodgerblue]',name+'[/COLOR]')
                    
                    if choice==1:
                        os.rename(os.path.join(addonstemp,name),os.path.join(ADDONS,name))
                        print"### Successfully installed: "+name
                        writefile.write('[COLOR=lime][SUCCESS][/COLOR] '+name+" - [COLOR=gold]This could not be verified but you accepted to install.[/COLOR][CR]")
                    
                    else:
                        print"### "+name+" Not installed"
                        writefile.write('[COLOR=red][FAILED][/COLOR] '+name+" - [COLOR=gold]This could not be verified and you chose not to instsall.[/COLOR][CR]")
                else:
                    print"### "+name+" Already exists on system"
...
def CB_Install_Results():
    ...
    successfile.close()
    Text_Boxes('Community Build Install Results',successcontent)
#---------------------------------------------------------------------------------------------------
...
def CB_Menu(welcometext):
    ...
    if not 'elc' in welcometext:
        o0oO('','[COLOR=orange]To access community builds you must be logged in[/COLOR]','settings','addon_settings','TOTALXBMC.png','','','Register at TotalXBMC.tv')
...
#    asciibase = '687474703a2f2f746f74616c78626d632e636f6d2f746f74616c7265766f6c7574696f6e2f436f6d6d756e6974795f4275696c64732f6275696c647570646174652e7068703f69643d2573'
#    BaseURL = binascii.unhexlify(asciibase) % (id)
    BaseURL = 'http://totalxbmc.com/totalrevolution/Community_Builds/buildupdate.php?id=%s' % (id)
    link    = Open_URL(BaseURL).replace('\n','').replace('\r','')
...
# Files and folders to exclude in backup process (FULL is for the full backup option - if enabled in settings)
    ...
    exclude_files_full = ["xbmc.log","xbmc.old.log","kodi.log","kodi.old.log",'.DS_Store','.setup_complete','XBMCHelper.conf','Thumbs.db']
    exclude_dirs       =  [AddonID, 'cache', 'system', 'addons', 'Thumbnails', "peripheral_data",'library','keymaps','script.tvguidedixie','script.module.metahandler','script.artistslideshow','ArtistSlideshow']
    exclude_files      = ["xbmc.log","xbmc.old.log","kodi.log","kodi.old.log","Textures13.db",'.DS_Store','.setup_complete','XBMCHelper.conf', 'advancedsettings.xml','Thumbs.db']
...
# If option to create a full build is ticked in settings
    ...
    choice = xbmcgui.Dialog().yesno("Do you want to include your addon_data folder?", 'This contains ALL addon settings including passwords. If you\'re intending on sharing this with others we stongly recommend against this unless all sensitive data has been manually removed.', yeslabel='Yes',nolabel='No')
...
# If it's the option to keep a full build then also create a guisettings.zip for that
    ...
    if guisuccess == 0:
        ...
        if mastercopy=='true':
            dialog.ok("Build Locations", 'Full Backup (only used to restore on this device): [COLOR=dodgerblue]'+myfullbackup, '[/COLOR]Universal Backup (can be used on any device): [COLOR=dodgerblue]'+backup_zip+'[/COLOR]')
        
        else:
            dialog.ok("Build Location", 'Universal Backup (can be used on any device):','[COLOR=dodgerblue]'+backup_zip+'[/COLOR]')
#---------------------------------------------------------------------------------------------------
# This creates the final menu showing build details, video and install link
def Community_Menu(url,video):
#    Base='687474703a2f2f746f74616c78626d632e636f6d2f746f74616c7265766f6c7574696f6e2f436f6d6d756e6974795f4275696c64732f636f6d6d756e6974795f6275696c64735f7072656d69756d2e7068703f69643d2573'
#    BaseURL=binascii.unhexlify(Base) % (url)
    check1             = 0
    ...
    o0oO('','[COLOR=yellow]IMPORTANT:[/COLOR] Install Instructions','','instructions_2','TOTALXBMC.png','','','')
...
#    if updateURL!='None':
    ...
    if localidcheck == head and localversioncheck != version:
        o0oO('','[COLOR=gold]----------------- UPDATE AVAILABLE ------------------[/COLOR]','None','','TOTALXBMC.png','','','')
        Add_Build_Dir('[COLOR=dodgerblue]1. Update:[/COLOR] Overwrite my existing library and profiles',downloadURL,'restore_community',iconimage,'','update',name,defaultskin,guisettingslink,artpack)
    ...
    if BaseURL.endswith("visibility=premium") or BaseURL.endswith("visibility=reseller_private") or BaseURL.endswith("visibility=reseller_openelec"):
        
        if (path!='') and (path!='fail') and (os.path.exists(path)):
            
            if BaseURL.endswith("visibility=reseller_openelec"):
                Add_Build_Dir('[COLOR=lime]Install - THIS WILL WIPE ANY EXISTING KODI SETTINGS[/COLOR]',downloadURL,'restore_openelec',iconimage,fanart,'',name,'','','')
            
            if BaseURL.endswith("visibility=premium") or BaseURL.endswith("visibility=reseller_private") and not BaseURL.endswith("visibility=reseller_openelec") and downloadURL!='None' and localidcheck != head:
...
#            if BaseURL.endswith("visibility=premium") or BaseURL.endswith("visibility=reseller_private") and updateURL!='None' and localidcheck != head:
#                Add_Build_Dir('[COLOR=orange]Update This Build (Latest Update: '+updateDate+')[/COLOR]',updateURL,'update_build',iconimage,fanart,updateDesc,name,defaultskin,guisettingslink,updateDate)
        
        if ((path!='') and not os.path.exists(path) and (path!='fail')):
            check1=1
            dialog.ok("Security check failed, contact box seller", 'This box cannot be identified as an official [COLOR=dodgerblue]'+I1IiiI+'[/COLOR] product. Please contact the seller you purchased this device from for more details.')
        ...
        if path=='':
            ...
            if BaseURL.endswith("visibility=reseller_openelec"):
                Add_Build_Dir('[COLOR=dodgerblue]Install - THIS WILL WIPE ANY EXISTING KODI SETTINGS[/COLOR]',downloadURL,'restore_openelec',iconimage,fanart,'',name,'','','')
    ...
    if check1==0:
        ...
        if guisettingslink=='None':
            ...
            if not BaseURL.endswith("visibility=reseller_openelec"):
                o0oO('','[COLOR=gold]---------- (OPTIONAL) Guisettings Fix ----------[/COLOR]','None','','TOTALXBMC.png','','','')
...
#                depascii='687474703a2f2f746f74616c78626d632e636f6d2f746f74616c7265766f6c7574696f6e2f4164646f6e506f7274616c2f646570656e64656e6379696e7374616c6c2e7068703f69643d2573'
#                BaseURL = binascii.unhexlify(depascii) % (requires)
                ...
                try:
                    ...
                    try:
                        ...
                        try:
                            ...
                            dialog.ok("Error downloading dependency", 'There was an error downloading [COLOR=dodgerblue]'+depname+'[/COLOR]. Please consider updating the add-on portal with details or report the error on the forum at [COLOR=lime][B]www.totalxbmc.tv[/COLOR][/B]')
                            status=0
                            modulestatus=0
                ...
                if status==1:
                    ...
                    time.sleep(1)
#                    asciiinc = '687474703a2f2f746f74616c78626d632e636f6d2f746f74616c7265766f6c7574696f6e2f4164646f6e506f7274616c2f646f776e6c6f6164636f756e742e7068703f69643d2573'
#                    incremental=binascii.unhexlify(asciiinc) % (requires)
...
def fixes():
#    linkasc='687474703a2f2f746f74616c78626d632e74762f746f74616c7265766f6c7574696f6e2f4164646f6e5f4669782f6164646f6e6669782e747874'
#    linkclean=binascii.unhexlify(linkasc)
    linkclean = 'http://totalxbmc.com/totalrevolution/AddonPortal/Addon_Fix/addonfix.txt'
...
#    ascbuild = '687474703a2f2f746f74616c78626d632e636f6d2f746f74616c7265766f6c7574696f6e2f4164646f6e506f7274616c2f736f727462792e7068703f736f7274783d6e616d6526757365723d2573266164756c743d2573262573'
#    buildsURL = binascii.unhexlify(ascbuild) % (username, adult, url)
    buildsURL  = 'http://totalxbmc.com/totalrevolution/AddonPortal/sortby_new.php?sortx=name&user=%s&adult=%s&%s' % (username, adult, url)
...
#    buildasci = '687474703a2f2f746f74616c78626d632e636f6d2f746f74616c7265766f6c7574696f6e2f436f6d6d756e6974795f4275696c64732f736f727462792e7068703f736f7274783d6e616d65266f72646572783d415343266164756c743d2573262573'
#    buildsURL = binascii.unhexlify(buildasci) % (adult, url)
    buildsURL  = 'http://totalxbmc.com/totalrevolution/Community_Builds/sortby.php?sortx=name&orderx=ASC&adult=%s&%s' % (adult, url)
    link       = Open_URL(buildsURL).replace('\n','').replace('\r','')
...
#    buildascii = '687474703a2f2f746f74616c78626d632e636f6d2f746f74616c7265766f6c7574696f6e2f4861726477617265506f7274616c2f736f727462792e7068703f736f7274783d4164646564266f72646572783d44455343262573'
#    buildsURL = binascii.unhexlify(buildascii) % (url)
...
def Grab_News(url):
#    newsasc = '687474703a2f2f746f74616c78626d632e636f6d2f746f74616c7265766f6c7574696f6e2f4c61746573744e6577732f736f727462792e7068703f736f7274783d6974656d5f64617465266f72646572783d44455343262573'
#    buildsURL = binascii.unhexlify(newsasc) % (url)
    ...
    for name, date, source, id in match:
        ...
        if "TotalXBMC" in source:
            o0oO('',name+'  ('+date+')',id,'news_menu','TOTALXBMC.png','','')
#---------------------------------------------------------------------------------------------------
#Function to populate the search based on the initial first filter
def Grab_Tutorials(url):
#    tutsobf = '687474703a2f2f746f74616c78626d632e636f6d2f746f74616c7265766f6c7574696f6e2f5475746f7269616c506f7274616c2f736f727462792e7068703f736f7274783d4e616d65266f72646572783d415343262573'
#    buildsURL = binascii.unhexlify(tutsobf) % (url)
    ...
    for name, about, id in match:
        o0oO('folder',name,id,'tutorial_final_menu','TotalXBMC_Guides.png','',about)
#---------------------------------------------------------------------------------------------------
...
#Copy to addon_data folder so profiles can be dealt with
    ...
    if local != 'fresh':
        choice2 = dialog.yesno("Do You Want To Keep Your Kodi Settings?", 'Would you like to keep your existing Kodi settings or would you rather wipe and install the ones created by the build author?', nolabel='Keep my settings',yeslabel='Replace my settings')
...
#    hwencr='687474703a2f2f746f74616c78626d632e636f6d2f746f74616c7265766f6c7574696f6e2f4861726477617265506f7274616c2f686172647761726564657461696c732e7068703f69643d2573'
#    BaseURL=binascii.unhexlify(hwencr) % (url)
    ...
    official_description  = str('[COLOR=gold]Available From: [/COLOR] [COLOR=lime]www.totalboxshop.tv[/COLOR][CR][CR][COLOR=dodgerblue]Added: [/COLOR]'+added+'[CR][COLOR=dodgerblue]Manufacturer: [/COLOR]'+manufacturer+'[CR][COLOR=dodgerblue]Supported Roms: [/COLOR]'+platform+'[CR][COLOR=dodgerblue]Chipset: [/COLOR]'+chipset+'[CR][COLOR=dodgerblue]CPU: [/COLOR]'+CPU+'[CR][COLOR=dodgerblue]GPU: [/COLOR]'+GPU+'[CR][COLOR=dodgerblue]RAM: [/COLOR]'+RAM+'[CR][COLOR=dodgerblue]Flash: [/COLOR]'+flash+'[CR][COLOR=dodgerblue]Wi-Fi: [/COLOR]'+wifi+'[CR][COLOR=dodgerblue]Bluetooth: [/COLOR]'+bluetooth+'[CR][COLOR=dodgerblue]LAN: [/COLOR]'+LAN+'[CR][CR][COLOR=yellow]About: [/COLOR]'+description+'[CR][CR][COLOR=yellow]Summary:[/COLOR][CR][CR][COLOR=dodgerblue]Pros:[/COLOR]    '+pros+'[CR][CR][COLOR=dodgerblue]Cons:[/COLOR]  '+cons+'[CR][CR][COLOR=yellow]Benchmark Results:[/COLOR][CR][CR][COLOR=dodgerblue]Boot Time:[/COLOR][CR]'+BootTime+'[CR][CR][COLOR=dodgerblue]Time taken to scan 1,000 movies (local NFO files):[/COLOR][CR]'+library+'[CR][CR][COLOR=dodgerblue]Copy 4,000 files (660.8MB) locally:[/COLOR][CR]'+CopyFiles+'[CR][CR][COLOR=dodgerblue]Copy a MP4 file (339.4MB) locally:[/COLOR][CR]'+CopyVideo+'[CR][CR][COLOR=dodgerblue]Ethernet Speed - Copy MP4 (339.4MB) from SMB share to device:[/COLOR][CR]'+EthernetTest+'[CR][CR][COLOR=dodgerblue]4k Playback:[/COLOR][CR]'+fourk+'[CR][CR][COLOR=dodgerblue]1080p Playback:[/COLOR][CR]'+teneighty+'[CR][CR][COLOR=dodgerblue]720p Playback:[/COLOR][CR]'+seventwenty+'[CR][CR][COLOR=dodgerblue]Audio Playback:[/COLOR][CR]'+DTS+'[CR][CR][COLOR=dodgerblue]Image Slideshow:[/COLOR][CR]'+Slideshow)
    official_description2 = str('[COLOR=gold]Availability: [/COLOR]Sorry this device is currently unavailable at [COLOR=lime]www.totalboxshop.tv[/COLOR][CR][CR][COLOR=dodgerblue]Added: [/COLOR]'+added+'[CR][COLOR=dodgerblue]Manufacturer: [/COLOR]'+manufacturer+'[CR][COLOR=dodgerblue]Supported Roms: [/COLOR]'+platform+'[CR][COLOR=dodgerblue]Chipset: [/COLOR]'+chipset+'[CR][COLOR=dodgerblue]CPU: [/COLOR]'+CPU+'[CR][COLOR=dodgerblue]GPU: [/COLOR]'+GPU+'[CR][COLOR=dodgerblue]RAM: [/COLOR]'+RAM+'[CR][COLOR=dodgerblue]Flash: [/COLOR]'+flash+'[CR][COLOR=dodgerblue]Wi-Fi: [/COLOR]'+wifi+'[CR][COLOR=dodgerblue]Bluetooth: [/COLOR]'+bluetooth+'[CR][COLOR=dodgerblue]LAN: [/COLOR]'+LAN+'[CR][CR][COLOR=yellow]About: [/COLOR]'+description+'[CR][CR][COLOR=yellow]Summary:[/COLOR][CR][CR][COLOR=dodgerblue]Pros:[/COLOR]    '+pros+'[CR][CR][COLOR=dodgerblue]Cons:[/COLOR]  '+cons+'[CR][CR][COLOR=gold]4k Playback:[/COLOR]  '+fourk+'[CR][CR][COLOR=gold]1080p Playback:[/COLOR]  '+teneighty+'[CR][CR][COLOR=gold]720p Playback:[/COLOR]  '+seventwenty+'[CR][CR][COLOR=gold]DTS Compatibility:[/COLOR]  '+DTS+'[CR][CR][COLOR=gold]Time taken to scan 100 movies:[/COLOR]  '+library)
    
    if description != '' and shop !='':
        o0oO('','[COLOR=yellow][Text Guide][/COLOR]  Official Description',official_description,'text_guide','TotalXBMC_Guides.png',FANART,'','')    
    if description != '' and shop =='':
        o0oO('','[COLOR=yellow][Text Guide][/COLOR]  Official Description',official_description2,'text_guide','TotalXBMC_Guides.png',FANART,'','')    
    if whufcleevid != 'None':
...
#    installobf='687474703a2f2f746f74616c78626d632e636f6d2f746f74616c7265766f6c7574696f6e2f436f6d6d756e6974795f4275696c64732f67756973657474696e67732e7068703f69643d2573'
#    BaseURL=binascii.unhexlify(installobf) % (url)
...
#    repoob='687474703a2f2f746f74616c78626d632e636f6d2f746f74616c7265766f6c7574696f6e2f4164646f6e506f7274616c2f646570656e64656e6379696e7374616c6c2e7068703f69643d2573'
#    BaseURL=binascii.unhexlify(repoob) % (repo_id)
    repostatus   = 1
    ...
    try:
        ...
        try:
            ...
            try:
                ...
                dialog.ok("Error downloading repository", 'There was an error downloading[CR][COLOR=dodgerblue]'+reponame+'[/COLOR]. Please consider updating the add-on portal with details or report the error on the forum at [COLOR=lime][B]www.totalxbmc.tv[/COLOR][/B]')
                repostatus=0
...
# If repository successfully installed add increment
    if repostatus==1:
        ...
        time.sleep(1)
#        asciiinc = '687474703a2f2f746f74616c78626d632e636f6d2f746f74616c7265766f6c7574696f6e2f4164646f6e506f7274616c2f646f776e6c6f6164636f756e742e7068703f69643d2573'
#        incremental=binascii.unhexlify(asciiinc) % (repo_id)
...
# This creates the final menu showing build details, video and install link
def News_Menu(url):
#    newsascii='687474703a2f2f746f74616c78626d632e636f6d2f746f74616c7265766f6c7574696f6e2f4c61746573744e6577732f4c61746573744e6577732e7068703f69643d2573'
#    BaseURL=binascii.unhexlify(newsascii) % (url)
...
#    notifyasc='687474703a2f2f746f74616c78626d632e636f6d2f746f74616c7265766f6c7574696f6e2f436f6d6d756e6974795f4275696c64732f6e6f746966793f726573656c6c65723d2573'
#    BaseURL=binascii.unhexlify(notifyasc) % (I1IiiI)
    
    BaseURL     = 'http://totalxbmc.com/totalrevolution/Community_Builds/notify?reseller=%s' % (I1IiiI)
    link        = Open_URL(BaseURL).replace('\n','').replace('\r','')
...
#-----------------------------------------------------------------------------------------------------------------    
def Popular():
#    poppacksenc = '687474703a2f2f746f74616c78626d632e74762f746f74616c7265766f6c7574696f6e2f4164646f6e5f5061636b732f6164646f6e7061636b732e747874'
#    poppacks = binascii.unhexlify(poppacksenc)
    poppacks = 'http://totalxbmc.tv/totalrevolution/Addon_Packs/addonpacks.txt'
...
def Register():
    dialog.ok("Register to unlock features", "To get the most out of this addon please register at the TotalXBMC forum for free. Visit [COLOR=lime]www.totalxbmc.tv[/COLOR] for more details.")
#---------------------------------------------------------------------------------------------------
...
#    resellerraw='687474703a2f2f746f74616c78626d632e636f6d2f746f74616c7265766f6c7574696f6e2f436f6d6d756e6974795f4275696c64732f726573656c6c65725f323f726573656c6c65723d257326746f6b656e3d2573266f70656e656c65633d2573'
#    BaseURL=binascii.unhexlify(resellerraw) % (I1IiiI, IIi1IiiiI1Ii, url)
    BaseURL          = 'http://totalxbmc.com/totalrevolution/Community_Builds/reseller_2?reseller=%s&token=%s&openelec=%s' % (I1IiiI, IIi1IiiiI1Ii, url)
    link             = Open_URL(BaseURL).replace('\n','').replace('\r','')
    pathmatch        = re.compile('path="(.+?)"').findall(link)
...
#Rename guisettings.xml to guinew.xml so we can edit without XBMC interfering.
    ...
    if exitfunction == 1:
        ...
        if video == 'libprofile' or video == 'updatelibprofile':
...
def Restore_OpenELEC(url,name):
    ...
    if choice == 0:
        ...
        time.sleep(2)
#        cbinc = '687474703a2f2f746f74616c78626d632e636f6d2f746f74616c7265766f6c7574696f6e2f436f6d6d756e6974795f4275696c64732f646f776e6c6f6164636f756e742e7068703f69643d2573'
#        incremental = binascii.unhexlify(cbinc) % (name)

        incremental = 'http://totalxbmc.com/totalrevolution/Community_Builds/downloadcount.php?id=%s' % (name)
        try:
            Open_URL(incremental)
        except: pass
...
def Restore_Option():
    Check_Local_Install()
    o0oO('','[COLOR=dodgerblue]RESTORE LOCAL BUILD[/COLOR]','url','restore_local_CB','Restore.png','','','Restore A Full System Backup')
...
def Restore_Zip_File(url):
    ...
    if 'Backup' in name:
        ...
        if 'Backup' in name:
            dialog.ok("Install Complete", 'To ensure the skin settings are set correctly Kodi will now close. If Kodi doesn\'t close please force close (pull power or force close in your OS - [COLOR=lime]DO NOT exit via Kodi menu[/COLOR])')
            Kill_XBMC()
...
def Show_Info(url):
#    inforaw='687474703a2f2f746f74616c78626d632e636f6d2f746f74616c7265766f6c7574696f6e2f436f6d6d756e6974795f4275696c64732f636f6d6d756e6974795f6275696c64732e7068703f69643d2573'
#    BaseURL=binascii.unhexlify(inforaw) % (url)
...
def Show_User_Info():
#    usrinfo = '687474703a2f2f746f74616c78626d632e636f6d2f746f74616c7265766f6c7574696f6e2f6c6f67696e2f6c6f67696e5f64657461696c732e7068703f757365723d257326706173733d2573'
#    BaseURL = binascii.unhexlify(usrinfo) % (username, password)
    ...
    dialog.ok('TotalXBMC Details for '+username,'Email: '+email,'Unread Messages: '+unread+'/'+messages,'Posts: '+posts)
#-----------------------------------------------------------------------------------------------------------------
...
#    tutraw = '687474703a2f2f746f74616c78626d632e636f6d2f746f74616c7265766f6c7574696f6e2f5475746f7269616c506f7274616c2f646f776e6c6f6164636f756e742e7068703f69643d2573'
#    incremental = binascii.unhexlify(tutraw) % (url)
    incremental = 'http://totalxbmc.com/totalrevolution/TutorialPortal/downloadcount.php?id=%s' % (url)
    Open_URL(incremental)
#    tutmenuraw='687474703a2f2f746f74616c78626d632e636f6d2f746f74616c7265766f6c7574696f6e2f5475746f7269616c506f7274616c2f7475746f7269616c64657461696c732e7068703f69643d2573'
#    BaseURL=binascii.unhexlify(tutmenuraw) % (url)
...
#    userinforaw = '687474703a2f2f746f74616c78626d632e636f6d2f746f74616c7265766f6c7574696f6e2f6c6f67696e2f6c6f67696e5f64657461696c732e7068703f757365723d257326706173733d2573'
    if login == 'true':
#        BaseURL = binascii.unhexlify(userinforaw) % (username, password)
        BaseURL   = 'http://totalxbmc.com/totalrevolution/login/login_details.php?user=%s&pass=%s' % (username, password)
#    else: BaseURL = binascii.unhexlify(userinforaw) % ('', '')
    else: BaseURL = 'http://totalxbmc.com/totalrevolution/login/login_details.php?user=%s&pass=%s' % ('','')
    link          = Open_URL(BaseURL).replace('\n','').replace('\r','')
                ...
                try:
                    ...
                    try:
                        ...
                        if trcheck == 'true':
                            ...
                            Categories2('','','','[COLOR=orange]NO INTERNET CONNECTION[/COLOR]')
#    else:
#        if AddonID == binascii.unhexlify('706c7567696e2e70726f6772616d2e746273'): # If Addon is plugin.program.tbs
#            checkurlraw='68747470733a2f2f746f74616c626f7873686f702e74762f5442532f5442535f436865636b2e747874'
#        if AddonID == binascii.unhexlify('706c7567696e2e70726f6772616d2e746f74616c696e7374616c6c6572'): # If Addon is plugin.program.totalinstaller
#            checkurlraw='687474703a2f2f746f74616c78626d632e636f6d2f746f74616c7265766f6c7574696f6e2f636865636b2e747874'
    if AddonID == 'plugin.program.tbs':
        checkurl='https://totalboxshop.tv/TBS/TBS_Check.txt'

    if AddonID == 'plugin.program.totalinstaller': # If Addon is plugin.program.totalinstaller
        checkurl='http://totalxbmc.com/totalrevolution/check.txt'
...
# Check the online version number and URL
    try:
...
# If reseller addon grab the contents of the addon.xml and replace new version with old contents
        if urlversion > addonversion and trcheck=='true':
            if AddonID == 'plugin.program.tbs':
                addonxml = xbmc.translatePath(os.path.join(ADDONS,AddonID,'addon.xml'))    # Grab existing data from addon.xml ready to inject in new
                localaddonversion  = open(addonxml, mode='r')
                content            = localaddonversion.read()
                localaddonversion.close()
            
                localaddonvermatch = re.compile('check="1" version="(.+?)"').findall(content)
                localnamematch     = re.compile('tbs" name="(.+?)"').findall(content)
                localsummarymatch  = re.compile('<summary lang="en">(.+?)"').findall(content)
                localdescmatch     = re.compile('<description>(.+?)</').findall(content)
                addonversion       = localaddonvermatch[0] if (len(localaddonvermatch) > 0) else '1.0'
                localname          = localnamematch[0] if (len(localnamematch) > 0) else 'Untitled'
                localsummary       = localsummarymatch[0] if (len(localsummarymatch) > 0) else 'The ultimate wizard'
                localdesc          = localdescmatch[0] if (len(localdescmatch) > 0) else 'The ultimate Kodi wizard!'

# Read the settings.xml into memory and later replace the new with existing
                time.sleep(1)
                settingsxml        = xbmc.translatePath(os.path.join(ADDONS,AddonID,'resources/settings.xml'))
                localsettings      = open(settingsxml, mode='r')
                content2           = localsettings.read()
                localsettings.close()
                localsettingsmatch = re.compile('id="openelec" type="bool"(.+?)<setting id="reseller"').findall(content2)

# Download and extract new version
            print"Downloading newer version"
            dp.create("Installing new version","Downloading ",'', 'Please Wait')
            downloader.download(urlzip, packages+'/'+AddonID+'.zip', dp)
            dp.update(0,"", "Extracting Zip Please Wait")
            Extract_all(packages+'/'+AddonID+'.zip', ADDONS, dp)
#                time.sleep(1)

# Replace addon.xml content
            if AddonID == 'plugin.program.tbs':
                a = open(addonxml).read()
                b = a.replace('YOUR_ADDON_NAME_HERE',localname).replace('ADD_DESCRIPTION_HERE',localdesc).replace('ADD_SUMMARY_HERE',localsummary)
                f = open(addonxml, mode='w')
                f.write(str(b))
                f.close()

# Read newly downloaded settings.xml content
                localsettings2 = open(settingsxml, mode='r')
                content3       = localsettings2.read()
                localsettings2.close()

# Replace newly downloaded settings.xml with old
                localsettingsmatch2 = re.compile('id="openelec" type="bool"(.+?)<setting id="reseller"').findall(content3)
                b                   = a.replace(localsettingsmatch2,localsettingsmatch)
                f                   = open(settingsxml, mode='w')
                f.write(str(b))
                f.close()
                xbmc.executebuiltin( 'UpdateLocalAddons' )
                xbmc.executebuiltin( 'UpdateAddonRepos' )    

#                unlockedraw='687474703a2f2f746f74616c78626d632e636f6d2f746f74616c7265766f6c7574696f6e2f756e6c6f636b65642e747874'
#                BaseURL=binascii.unhexlify(unlockedraw)
...
# Check for a new startup video
    ...
    # THIS CAN BE CHANGED FOR THE TIMESTAMP FUNCTION
...
# if os.path.exists(os.path.join(ADDONS,'plugin.program.TotalRevolution')):
    # choice=dialog.yesno('Old Version Detected','An old version of the Total Revolution add-on that is no longer supported has been found on your system. Would you like to remove the old version?')
    # if choice == 1:
        # try:
            # shutil.rmtree(os.path.join(ADDONS,'plugin.program.TotalRevolution'))
            # xbmc.executebuiltin( 'UpdateLocalAddons' )
            # xbmc.sleep(1000)
            # xbmc.executebuiltin( 'UpdateAddonRepos' )
        # except: pass
if mode == None : Video_Check()
#elif mode == None : Categories2('','','','')
...
elif mode == 'cb_install_results' : CB_Install_Results()
...
elif mode == 'restore_local_CB'   : Restore_Local_Community()
