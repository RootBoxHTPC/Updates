...
def Notify_Check():
    ...
    if isplaying == 0:
        ...
        BaseURL='http://totalxbmc.com/totalrevolution/Community_Builds/notify?reseller=%s' % (sellername)
...
def CB_Notify_Check():
    ...
    if localidcheck != 'None':
        ...
        if isplaying == 0:
            ...
            BaseURL='http://totalxbmc.com/totalrevolution/Community_Builds/notify?id=%s' % (localidcheck)
            link = Open_URL(BaseURL).replace('\n','').replace('\r','')
            notifymatch = re.compile('notify="(.+?)"').findall(link)
            notifymsg = notifymatch[0] if (len(notifymatch) > 0) else 'No news items available'
            datematch = re.compile('date="(.+?)"').findall(link)
